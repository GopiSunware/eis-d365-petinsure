...
class AzureCostService:
    ...
    def _get_scope(self) -> str:
        ...
        if settings.AZURE_COST_MANAGEMENT_SCOPE:
            return settings.AZURE_COST_MANAGEMENT_SCOPE
    ...
    async def _get_resource_groups(self) -> List[str]:
        ...
        if not settings.azure_resource_groups:
            return []
        ...
        try:
            url = f"{self.COST_MANAGEMENT_API}/subscriptions/{settings.AZURE_SUBSCRIPTION_ID}/resourcegroups?api-version=2021-04-01"
            ...
            async with httpx.AsyncClient() as client:
                ...
                # Filter by patterns
                patterns = settings.azure_resource_groups
                filtered = []
                for rg in all_rgs:
                    rg_lower = rg.lower()
